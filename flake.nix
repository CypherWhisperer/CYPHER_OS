...
# ──────────────────────────────────────────────────────────────────────────────
#
# Entry point for the cypherOS configuration flake.
#
...
# six months from now uses the same nixpkgs unless `nix flake update` is
# explicitly ran.
#
...

{
  ...
  # plus overlays (./flake/overlays) for extra packages.
  # ────────────────────────────────────────────────────────────────────────────
  ...
  inputs = {
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Home-manager: follows nixpkgs exactly (same revision) to avoid version
    # skew.
    #
    # "follows" is a flake deduplication mechanism that says "use the same
    # nixpkgs input as the parent flake, don't fetch your own copy."
    #
    # Ensures input Flakes and CypherOS all evaluate against the same nixpkgs
    # revision. Without this, Nix would fetch and evaluate a second
    ...
    # This is the escape hatch for extensions not packaged in nixpkgs — no
    # manual sha256 hunting, no hash-mismatch failures at build time.
    # Update all extension hashes in one shot with:
    #
    ...
    # pre-compiles the theme at build time (bypassing the read-only store
    # problem that breaks the nixpkgs catppuccin-vsc extension).
    # ──────────────────────────────────────────────────────────────────────────
    catppuccin = {
  ...
  #
  # The @ binding captures ALL inputs into a single attribute set called
  # `inputs`.
  # ────────────────────────────────────────────────────────────────────────────
  outputs =
    {
