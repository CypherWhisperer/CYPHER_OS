...
let
  ...
  # version. This keeps the system-wide installation clean while each project
  # can declare its own exact runtime contract.
  #
  ...
  phpEnv = pkgs.php.buildEnv {
    extensions = (
      ...
      ++ (with all; [
        ...
        # DEBUGGING.
        # ──────────────────────────────────────────────────────────────────────
        # §XDEBUG-NOTE: see extraConfig below for the full constraint
        # documentation.
        # ──────────────────────────────────────────────────────────────────────
        xdebug
        ...
        # ──────────────────────────────────────────────────────────────────────
        # CACHING / MESSAGE QUEUES (i.e., Laravel cache, jobs, events).
        # ──────────────────────────────────────────────────────────────────────
        ...
        # ──────────────────────────────────────────────────────────────────────
        # NOTE: ⚠ CONFLICT: xdebug and pcov cannot be loaded simultaneously.
        # ──────────────────────────────────────────────────────────────────────
...
in
{
  options.cypher-os.pkgs.dev.languages = {
    enable = mkEnableOption "CypherOS Programming Languages suite and configurations.";

    bash.enable = mkEnableOption "Bash scripting Language tooling and configuration for CypherOS.";
    cCpp.enable = mkEnableOption "C/C++ Low level (systems) programming Language tooling and configuration for CypherOS.";
    dart.enable = mkEnableOption "Dart Mobile development programming Language tooling and configuration for CypherOS.";
    jsTs.enable = mkEnableOption "JS/TS Web programming Language tooling and configuration for CypherOS.";
    kotlin.enable = mkEnableOption "Kotlin Mobile development programming Language tooling and configuration for CypherOS.";
    lua.enable = mkEnableOption "Lua programming Language tooling and configuration for CypherOS.";
    nix.enable = mkEnableOption "ix Programming Language tooling and configuration for CypherOS.";
    python.enable = mkEnableOption "Python General Purpose Programming Language tooling and configuration for CypherOS.";
    rust.enable = mkEnableOption "Rust Memory-safe Programming Language tooling and configuration for CypherOS.";
    zig.enable = mkEnableOption "Zig Programming Language tooling and configuration for CypherOS.";
    ...
    go = {
      ...
      telemetry = mkOption {
        ...
        description = ''
          ...
          collects counters on disk without uploading. "on" additionally uploads
          to telemetry.go.dev weekly. "off" disables telemetry entirely.
        '';
      };
    };
    ...
    d2 = {
      ...
      themeId = lib.mkOption {
        ...
        # ──────────────────────────────────────────────────────────────────────
        # "Dark Mauve" — matches the Catppuccin Mocha Mauve neutral palette
        # ──────────────────────────────────────────────────────────────────────
        default = 200;
      ...
      # ────────────────────────────────────────────────────────────────────────
      # Below are valid d2 CLI flags that just aren't in active use yet:
      # ────────────────────────────────────────────────────────────────────────
