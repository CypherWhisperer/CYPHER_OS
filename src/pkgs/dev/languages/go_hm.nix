...
in
{
  ...
  config = lib.mkIf (cfg.enable && cfg.go.enable) {
    programs.go = {
      enable = true;
      package = cfg.go.package; # or  pkgs.go_1_26 / pkgs.go_1_27 as needed

      # ────────────────────────────────────────────────────────────────────────
      # vendored GOPATH packages (rarely needed with modules)
      # ────────────────────────────────────────────────────────────────────────
      # packages = { ... };
      ...
      env = {
        ...
        # ──────────────────────────────────────────────────────────────────────
        GOPATH = "${config.home.homeDirectory}/${cfg.go.goPath}";
        GOPRIVATE = cfg.go.goPrivate;
      ...
      telemetry.mode = cfg.go.telemetry;
    ...
    #
    # Kept as a flat list rather than threaded through `cfg.go.extraTools`
    # defaults so it's visible in one place what "Go support" actually pulls in.
    # ──────────────────────────────────────────────────────────────────────────
    home.packages =
      with pkgs;
      [
        ...
        # ──────────────────────────────────────────────────────────────────────
        # struct tag add/remove — used by both ide' code actions
        # ──────────────────────────────────────────────────────────────────────
        gomodifytags
        ...
        # ──────────────────────────────────────────────────────────────────────
        # Configurable style linter, especially useful when a project adopts it.
        # ──────────────────────────────────────────────────────────────────────
        #revive
        ...
        # but golangci-lint adds format-on-run (`golangci-lint fmt`) and
        # org-wide .golangci.yml policy support
        #
        # It is a repo-level multi-linter runner; best controlled by project
        # `.golangci.yml`
        #
        ...
        # ──────────────────────────────────────────────────────────────────────
        # air: live-reload runner (for long-running Go services) —
        #  for service dev. Only relevant once an actual long-running service
        # (vs. CLI tools/libraries) is under active development.
        # ──────────────────────────────────────────────────────────────────────
        #air
