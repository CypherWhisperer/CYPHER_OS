# ──────────────────────────────────────────────────────────────────────────────
# src/pkgs/productivity/obsidian.nix
# ──────────────────────────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────────────────────────
# ARCHITECTURE:
# ──────────────────────────────────────────────────────────────────────────────
# HM writes symlinks into <vault>/.obsidian/ from the Nix store.
# Obsidian cannot write back to those files at runtime — any plugin that
# persists to data.json must have its settings declared here under `settings`
# (mapped to its data.json).
#
# Any plugin desired active MUST be declared here.
# Do not mix declarative and manual plugin management for the same vault.
#
# ──────────────────────────────────────────────────────────────────────────────
# COMMUNITY PLUGINS:
# ──────────────────────────────────────────────────────────────────────────────
# communityPlugins accepts a list of Nix derivations (packages), not plain
# strings. Each plugin is a stdenv.mkDerivation that fetches the plugin source
# from GitHub. There is no pkgs.obsidianPlugins.* attribute set in nixpkgs —
# plugins are defined locally as callPackage expressions.
#
# ──────────────────────────────────────────────────────────────────────────────
# CATPPUCCIN THEMING:
# ──────────────────────────────────────────────────────────────────────────────
...
# injects a Catppuccin theme derivation into
...
let
  ...
  # expected to be moved under ./obsidian/ once ./obsidian.nix is modularized
  # ────────────────────────────────────────────────────────────────────────────
  homeRelativeVaultPath =
    lib.removePrefix "${cypherOsConstants.homeDirectory}/" cypherOsConstants.obsidianVaultRoot;

  # ────────────────────────────────────────────────────────────────────────────
  # ── Plugin helper:
  # ────────────────────────────────────────────────────────────────────────────
  # Inline mkDerivation for small plugins. For larger ones, move to a
  # separate file and use pkgs.callPackage.
  #
  ...
  #
  # Obsidian loads main.js at runtime via Node's require().The compiled main.js
  # is NOT present in the plugin's git repository — it is a build artifact
  # published to GitHub Releases.
  #
  # Fetching the source tree via fetchFromGitHub therefore produces a derivation
  # with no main.js, and Obsidian throws "failed to load plugin" silently at
  # startup despite the symlink existing.
  #
  ...
  #
  # nix-prefetch-url \
  #  https://github.com/<owner>/<repo>/releases/download/<version>/manifest.json
  #
  # nix-prefetch-url \
  #  https://github.com/<owner>/<repo>/releases/download/<version>/styles.css
  #
  # ────────────────────────────────────────────────────────────────────────────
  ...
  # ── Theme helper:
  # ────────────────────────────────────────────────────────────────────────────
  # NOTE: THEME MANAGEMENT DELEGATED TO (catppuccin/nix). Prior implementation:
  # ────────────────────────────────────────────────────────────────────────────
  #mkTheme =
  #  {
  #    pname,
  ...
  # the new version tag, and convert with (check header comment block above).
  # ────────────────────────────────────────────────────────────────────────────
...
in
{
  ...
  config = lib.mkIf (cfg.enable && cfg.obsidian.enable) {
    programs.obsidian = {
      enable = true;
      package = pkgs.obsidian;

      # ────────────────────────────────────────────────────────────────────────
      # defaultSettings apply to all vaults. Vault-specific settings.* override
      # these. defaultSettings only has the typed sub-options the module
      #
      # explicitly defines: {
      #  appearance, app, corePlugins, communityPlugins, themes, cssSnippets,
      #  hotkeys, extraFiles
      # }.
      # ────────────────────────────────────────────────────────────────────────
      defaultSettings = {
        appearance = {
          ...
          # NOTE: THEME MANAGEMENT DELEGATED TO (catppuccin/nix).
          ...
          # Font Management.
          # ────────────────────────────────────────────────────────────────────
          # Note textFontFamily — Obsidian has three separate font slots:
          # interface, text (body), and monospace.
          #
          # If you only two are set, the third resets to system default.
          # ────────────────────────────────────────────────────────────────────
        ...
        app = {
          ...
          # ────────────────────────────────────────────────────────────────────
          # Constrains line width in reading/source view — reduces eye-tracking
          # fatigue on wide monitors.
          # ────────────────────────────────────────────────────────────────────
          "readableLineLength" = true; # constrain line width in reading mode

          # ────────────────────────────────────────────────────────────────────
          # Hides line numbers — cleaner prose environment; enable if heavy
          # structural editing is part of a workflow.
          # ────────────────────────────────────────────────────────────────────
          "showLineNumber" = false;

          # ────────────────────────────────────────────────────────────────────
          # Opens files in Live Preview by default — immediate rendered feedback
          # without a split pane.
          # ────────────────────────────────────────────────────────────────────
          "defaultViewMode" = "live"; # source | preview | live-preview

          # ────────────────────────────────────────────────────────────────────
          # Enables the hybrid source+preview editor engine; required for
          ...
          # ────────────────────────────────────────────────────────────────────
          # Uses spaces rather than hard tab characters — consistent with
          # tabSize above.
          # ────────────────────────────────────────────────────────────────────
          "useTab" = false;

          # ────────────────────────────────────────────────────────────────────
          # Collapses indented list items — useful in long structured notes;
          # toggle per note as needed.
          # ────────────────────────────────────────────────────────────────────
          "foldIndent" = true;

          # ────────────────────────────────────────────────────────────────────
          # Collapses headings on open — reduces visual noise in long docs.
          # ────────────────────────────────────────────────────────────────────
          "foldHeading" = false;

          # ────────────────────────────────────────────────────────────────────
          # Hides YAML frontmatter block in reading mode — cleaner output for
          # prose-heavy notes.
          # ────────────────────────────────────────────────────────────────────
          "showFrontmatter" = false;

          # ────────────────────────────────────────────────────────────────────
          # Single newline renders as a line break (GFM-compatible) — keeps repo
          # docs portable across renderers.
          # ────────────────────────────────────────────────────────────────────
          "strictLineBreaks" = false;

          # ────────────────────────────────────────────────────────────────────
          # Enables spell checking — useful for journal and prose entries across
          # all vault concerns.
          # ────────────────────────────────────────────────────────────────────
          "spellcheck" = true;

          # ────────────────────────────────────────────────────────────────────
          # Spell check language set — extend if your vault spans multiple
          # written languages.
          # ────────────────────────────────────────────────────────────────────
          "spellcheckLanguages" = [ "en" ];

          # ────────────────────────────────────────────────────────────────────
          # Auto-indents continuation of list items on Enter — prevents manual
          # re-indenting in nested lists.
          # ────────────────────────────────────────────────────────────────────
          "smartIndentList" = true;

          # ────────────────────────────────────────────────────────────────────
          # Converts pasted HTML to Markdown on paste — reduces cleanup when
          # pulling content from browsers.
          # ────────────────────────────────────────────────────────────────────
          "autoConvertHtml" = true;

          # ────────────────────────────────────────────────────────────────────
          # Vim keybindings — disabled; toggle if modal editing ever becomes
          # preferable.
          # ────────────────────────────────────────────────────────────────────
          "vimMode" = false;

          # ────────────────────────────────────────────────────────────────────
          # Canvas: snap cards to grid by default — cleaner spatial layout,
          # especially for research boards where structure matters.
          # Toggle off per canvas at runtime.
          # ────────────────────────────────────────────────────────────────────
          "canvasSnap" = true;
          ...
          # ────────────────────────────────────────────────────────────────────
          # Writes standard Markdown links instead of [[wikilinks]] — critical
          # for portability outside Obsidian.
          # ────────────────────────────────────────────────────────────────────
          "useMarkdownLinks" = true;

          # ────────────────────────────────────────────────────────────────────
          # Relative link paths — keeps links valid in any renderer
          # (GitHub, mdBook, VSCode preview).
          # ────────────────────────────────────────────────────────────────────
          "newLinkFormat" = "relative";

          # ────────────────────────────────────────────────────────────────────
          # Auto-updates links when a file is renamed — prevents silent broken
          # references across the vault.
          # ────────────────────────────────────────────────────────────────────
          "alwaysUpdateLinks" = true;

          # ────────────────────────────────────────────────────────────────────
          # Opens new tabs adjacent to the active one — preserves spatial
          # context while navigating.
          # ────────────────────────────────────────────────────────────────────
          "openNextToActiveLeaf" = true;

          # ────────────────────────────────────────────────────────────────────
          # New tab steals focus on open — direct navigation flow without manual
          # click-to-focus.
          # ────────────────────────────────────────────────────────────────────
          "focusNewTab" = true;

          # ────────────────────────────────────────────────────────────────────
          # Allows Obsidian to traverse symlinks into directories outside the
          # vault root.
          # ────────────────────────────────────────────────────────────────────
          "followSymlinks" = true;
          ...
          # ────────────────────────────────────────────────────────────────────
          # Applied during vault traversal at startup — prevents Obsidian from
          # placing inotify watches on directories that either contain
          # permission-restricted files (e.g. .git/objects/pack at 444) or would
          # generate excessive noise.
          #
          # Matches against path *components* (substring match on each directory
          # name). A filter of ".git" blocks any directory named exactly ".git"
          # at any depth.
          #
          # ────────────────────────────────────────────────────────────────────
          # Sources:
          ...
          "userIgnoreFilters" = [
            ...
            # ── Version control:
            # ──────────────────────────────────────────────────────────────────
            # Git internals — pack objects are 444 read-only; EACCES root cause
            # ──────────────────────────────────────────────────────────────────
            ".git"
            ...
            # ──────────────────────────────────────────────────────────────────
            # npm/yarn/pnpm deps — can contain thousands of .md files
            # ──────────────────────────────────────────────────────────────────
            "node_modules"
            ...
            # ── Rust:
            # ──────────────────────────────────────────────────────────────────
            "target" # Cargo build artifacts — can be gigabytes with binaries
            ...
            # ──────────────────────────────────────────────────────────────────
            # nix build symlink output — points into /nix/store (read-only)
            # ──────────────────────────────────────────────────────────────────
            "result"

            # ──────────────────────────────────────────────────────────────────
            # devenv state dir — contains symlinks into Nix store + DB files
            # ──────────────────────────────────────────────────────────────────
            ".devenv"

            # ──────────────────────────────────────────────────────────────────
            # direnv cache — nix-direnv writes shell env here
            # ──────────────────────────────────────────────────────────────────
            ".direnv"
            ...
            # ──────────────────────────────────────────────────────────────────
            # VS Code workspace settings (unlikely in vault but defensive)
            # ──────────────────────────────────────────────────────────────────
            ".vscode"
            ...
            # ── OS and system:
            # ──────────────────────────────────────────────────────────────────
            ".DS_Store" # macOS metadata (irrelevant on NixOS; harmless to list)
          ...
          # ────────────────────────────────────────────────────────────────────
          # Dropped/pasted attachments land in ./assets relative to the current
          # note — keeps repos clean.
          # ────────────────────────────────────────────────────────────────────
          "attachmentFolderPath" = "./assets";

          # ────────────────────────────────────────────────────────────────────
          # New notes are created in the same folder as the currently active
          # note — contextually appropriate.
          # ────────────────────────────────────────────────────────────────────
          ...
          # ────────────────────────────────────────────────────────────────────
          # Confirms before deleting — essential guard when editing files that
          # are symlinked into live repos.
          # ────────────────────────────────────────────────────────────────────
          "promptDelete" = true;

          # ────────────────────────────────────────────────────────────────────
          # Sends deleted files to the system trash — recoverable and keeps the
          # vault directory clean.
          # ────────────────────────────────────────────────────────────────────
          "trashOption" = "system";
        };
        ...
        # the HM module — listed explicitly here allowing commenting out what
        # isn't used and have an auditable record.
        # ──────────────────────────────────────────────────────────────────────
        corePlugins = [
          # ────────────────────────────────────────────────────────────────────
          # ── Navigation & UI:
          ...
          # Each entry in defaultSettings.corePlugins accepts a settings field
          # of type nullOr (attrsOf anything) — core plugin settings are
          # declared inline on the plugin entry itsel.
          # ────────────────────────────────────────────────────────────────────
          {
            name = "daily-notes";
            settings = {
              ...
              "format" = "YYYY_MM_DD";
              "template" = ""; # path to a template note if needed;

              # ────────────────────────────────────────────────────────────────
              # true = opens today's note automatically on Obsidian launch
              # ────────────────────────────────────────────────────────────────
              "autorun" = false;
            };
          }
          ...
          # ────────────────────────────────────────────────────────────────────
          # Obsidian's native database/property system (new in 1.8)
          # ────────────────────────────────────────────────────────────────────
          "bases"

          # ────────────────────────────────────────────────────────────────────
          # (frontmatter properties panel) surfaces YAML frontmatter as visual
          # property panel.
          # ────────────────────────────────────────────────────────────────────
          "properties"

          # ────────────────────────────────────────────────────────────────────
          # built-in browser pane — needed for webpage cards on canvas
          # ────────────────────────────────────────────────────────────────────
          "webviewer"
        ...
        #
        # `settings` maps directly to the plugin's
        ...
        communityPlugins = [
          ...
          # Without this, the Catppuccin options panel in Preferences →
          # Appearance is empty and the theme renders with its default palette
          # only.
          # ────────────────────────────────────────────────────────────────────
          {
            ...
            # ──────────────────────────────────────────────────────────────────
            settings = {
              "catppuccin@@accent" = catppuccinAccent;
          ...
          # ────────────────────────────────────────────────────────────────────
          {
            ...
            settings = {
              ...
              "enableInlineDataviewJs" = false; # enable only if inline DQL used
...

          {
            ...
            settings = {
              ...
              # ────────────────────────────────────────────────────────────────
              # ms after last keystroke before re-render (plugin min: 100)
              # ────────────────────────────────────────────────────────────────
              "debounce" = 500;

              # ────────────────────────────────────────────────────────────────
              # "path" left unset deliberately: Obsidian on NixOS/Home Manager
              # inherits $PATH from the session, and home.sessionVariables in
              # `pkgs/dev/languages/d2.nix` already puts `d2` on it — so the
              # plugin's own PATH probing (it checks common install locations +
              # $PATH) finds it without help. Only set this if Obsidian is
              # sandboxed/launched with a stripped PATH
        ...
        # ── CSS snippets:
        # ──────────────────────────────────────────────────────────────────────
        # Inline snippets (text =) or file references (source =).
        # These are placed in .obsidian/snippets/ and can be toggled in
        ...
        # ── Hotkeys:
        # ──────────────────────────────────────────────────────────────────────
        # Maps to hotkeys.json. Format:
        #  "plugin-id:command-id" = [ { modifiers = []; key = ""; } ];
        #
        ...
        hotkeys = {
          ...
          # ────────────────────────────────────────────────────────────────────
          # canvas:convert-to-file has no default — assign if inline text cards
          # used heavily
          #
          # canvas:zoom-to-fit — already defaults to Shift+1 on most systems
          # override if needed
          # ────────────────────────────────────────────────────────────────────
        };
      };
      ...
      # if it doesn't exist. Actual notes inside are never touched — only
      ...
      vaults = {
        # ──────────────────────────────────────────────────────────────────────
        # Vault name (the Nix attribute key) is arbitrary — it's just an
        # identifier within the HM module. The `target` is what actually
        # matters: it's the path relative to $HOME that HM will manage
        # .obsidian/ inside.
        #
        # If coming from an already existing vault with hand-crafted .obsidian/
        # (appearance.json, app.json, core-plugins.json, graph.json,
        # workspace.json), switching to the declarative module will OVERWRITE
        # those files with HM-managed symlinks. Back up .obsidian/ before the
        # first `home-manager switch` with this enabled:
        #
        #   cp -r ~/PATH/TO/YOUR/VAULT/.obsidian \
        #          ~/PATH/TO/YOUR/VAULT/.obsidian.bak
        #
        # Then inspect the backed-up JSONs to port your existing appearance/app
        # settings into the `settings` block here if you want to preserve them
        # declaratively.
        # ──────────────────────────────────────────────────────────────────────
        my-obsidian-notes = {
          # ────────────────────────────────────────────────────────────────────
          # NOTE: The assertion related to this is currently in ./defaults.nix
          # but is expected to be moved under ./obsidian/ once ./obsidian.nix is
          # modularized
          # ────────────────────────────────────────────────────────────────────
          target = homeRelativeVaultPath; # "DATA/FILES/PROJECTS/PRIVATE/OBSIDIAN_NOTES";
