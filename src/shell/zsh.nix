...
#   - Aliases (navigation, git, dev, system conveniences, etc)
...
# ──────────────────────────────────────────────────────────────────────────────
#   - The fzf *package* itself — declared in ../pkgs/cli/
#   - The keychain *package*  — declared in ../pkgs/cli/
#   - The fastfetch *package* — declared in ../pkgs/cli/
#   - Font packages            — declared in ../fonts/
#
# ──────────────────────────────────────────────────────────────────────────────
# ZDOTDIR NOTE:
# ──────────────────────────────────────────────────────────────────────────────
...
#   before the shell starts, so ZDOTDIR resolves correctly per profile
#   automatically. No manual ZDOTDIR wiring is needed here.
# ──────────────────────────────────────────────────────────────────────────────
...
let
  ...
  primaryUsername = cypherOsConstants.username;
  homeDirectory = cypherOsConstants.homeDirectory;
  activeHostName = cypherOsConstants.activeHostName;
  p10kThemeFiles = cypherOsConstants.zshPowerLevel10kThemeFile;
in
{
  imports = [ ./options.nix ];
  ...
  config = lib.mkIf (cfg.enable && cfg.zsh.enable) {
    ...
    home.packages = with pkgs; [
      ...
      # environment, and unload them if the .envrc is not reachable from the
      # current path anymore.
      #
    ...
    programs.zsh = {
      ...
      shellAliases = {
        ...
        # ──────────────────────────────────────────────────────────────────────
        "hms" =
          "clear && home-manager switch -b hm-bk --flake $HOME/CYPHER_OS#${primaryUsername}@${activeHostName}";

        "nfu" = "clear && nix flake update --flake $HOME/CYPHER_OS";
        "nrs" =
          "clear && sudo nixos-rebuild switch --install-bootloader --flake $HOME/CYPHER_OS#${activeHostName} --impure";
        # Safe rebuild — always reinstalls bootloader
        "nrbt" =
          "clear && sudo nixos-rebuild boot --install-bootloader --flake $HOME/CYPHER_OS#${activeHostName} --impure";
        # Build only — for testing evaluation without committing
        "nrbd" = "clear && nixos-rebuild build --flake $HOME/CYPHER_OS#${activeHostName} --impure";
        ...
        # shellAliases =.
        #
        # Nix doesn't support // merging inline there, but since config is in
        # scope (the module receives it as an arg), use lib.mkMerge at the
        # config level —  except the whole config block is already one attrset.
        #
        # The cleanest fit given existing structure:
        #   add a lib.optionalAttrs call at the bottom of the shellAliases
        #   attrset as below:
        #
        ...
        #              Ctrl-C to quit. Change baudrate to match Serial.begin()
        #              in sketch.
        #
      ...
      // lib.optionalAttrs config.cypher-os.pkgs.dev.arduino.enable {
        "ard-build" = "arduino-cli compile --fqbn ${config.cypher-os.pkgs.dev.arduino.fqbn}";
        "ard-upload" =
          "arduino-cli compile --fqbn ${config.cypher-os.pkgs.dev.arduino.fqbn} --upload --port";
